...
const createUser = postTool({
  ...
  description: 'Create a new user account from an invitation token and password.',
  schema: z.object({
...
const generate2FASecret = getTool({
  ...
  description: 'Generate a new 2FA secret for the current user to set up an authenticator app.',
  schema: z.object({}),
...
const verify2FASetup = postTool({
  ...
  description: 'Verify and complete the 2FA setup by providing a PIN from the authenticator app.',
  schema: z.object({
...
const verifyLogin2FA = postTool({
  ...
  description: 'Verify a 2FA PIN during login to complete the authentication process.',
  schema: z.object({
    pin: z.string().min(6).describe('The 6-digit PIN from the authenticator app'),
...
const disable2FA = postTool({
  ...
  description: "Disable two-factor authentication for the current user's account.",
  schema: z.object({}),
